...
def main_GUI():
    ...
    next_image = Image.open('Additional/icons/next-button (1).png').resize((40, 40))
    previous_image = Image.open('Additional/icons/next-button.png').resize((40, 40))
    volume_image = Image.open('Additional/icons/icons8-sound-speaker-48.png').resize((40, 40))
    add_new_song_image = Image.open('Additional/icons/button.png').resize((60, 60))
    delete_song_image = Image.open('Additional/icons/trash.png').resize((60, 60))
    move_song_image = Image.open('Additional/icons/move-file.png').resize((60, 60))
    ...
    delete_folder_image = Image.open('Additional/icons/delete_folder.png').resize((60, 60))
...
def move_song_window():
    ...
    try:
        ...
        # Налаштування основних параметрів вікна та стилів
        root.move_song = tk.Toplevel(root)
        root.move_song.title("Move Song")
        # root.move_song.iconbitmap('media/47240.ico')
        ...
        # Створення основного сектору GUI
        ...
        lockal_dir_list = os.listdir('Songs')
        for folder in lockal_dir_list:
        ...
        move_song_btn = Button(side_frame, text='Move Song', style='My.TButton', command=lambda: move_song(lockal_dir_list[folder_list_ms.curselection()[0]], old_folder, song_name))
...
def cloud_sync_window():
    ...
    # root.cloud_sync.iconbitmap('media/47240.ico')
    ...
    delete_checkbox = tk.Checkbutton(main_frame, image=root.off_btn_image, selectimage=root.on_bnt_image, indicatoron=False,
                         onvalue=1, offvalue=0, variable=delete_flag, relief='flat', bd=0, bg=bg_color, activebackground=bg_color, selectcolor=bg_color)
...
def download_song_window():
    global entry
    # Закриття вікна, якщо воно вже відкрите
    ...
    # root.download_song.iconbitmap('media/47240.ico')
    ...
    create_dir_btn = Button(side_frame, text='Download', style='My.TButton', command=lambda: create_thread_youtube_audio_download(youtube_audio_download, video_url=entry.get(), download_dir=lockal_dir_list[folder_list_yt.curselection()[0]], playlist=playlist_checkbox))
...
def create_directory_window():
    ...
    # root.add_dir.iconbitmap('media/47240.ico')
    ...
    entry = Entry(main_frame, width=40, font=('Miriam Libre', 14), foreground='deep pink2')
    entry.grid(row=1, column=0, padx=20, pady=10, sticky='news')

    create_dir_btn = Button(main_frame, text='Create', style='My.TButton', command=lambda: create_directory(entry.get()))
